...
def init_db():
    with engine.connect() as conn:
        ...
        conn.execute(sa.text("CREATE TABLE IF NOT EXISTS dms (id SERIAL PRIMARY KEY, from_nickname TEXT, to_nickname TEXT, text TEXT, time TEXT);"))
        ...
        conn.execute(sa.text("CREATE TABLE IF NOT EXISTS questions (id SERIAL PRIMARY KEY, key TEXT, q TEXT, options TEXT, ans TEXT, exp TEXT);"))
    ...
    with DBSession() as db:
        if request.method=="POST":
            if "new_post" in request.form:
                db.execute(sa.text("INSERT INTO posts (nickname, name, text, likes, comments) VALUES (:u, :n, :t, '[]', '[]')"),
                           {"u": nickname, "n": user["name"], "t": request.form["post"]})
...
def friends(nickname, user):
    with DBSession() as db:
        ...
        # FIX: Get all data BEFORE closing DB
    ...
    friends_html = "".join([f"<div class=card>👤 {name}</div>" for name in friends_names]) or "<p>No friends yet</p>"
...
def dm_list(nickname, user):
    with DBSession() as db:
        ...
        for f in user['friends']:
            ...
            friends_html += f"<a class=friend-card href=/dm/{f}><div class=friend-avatar>{initial}</div><div style='flex:1'><b>{name}</b><br><small>@{f}</small></div><button class='btn blue' style='width:auto;padding:8px 15px'>Message</button></a>"
...
def dm_chat(nickname, user, to_nickname):
    ...
    with DBSession() as db:
        ...
        # FIX: Pre-fetch all names
        msgs = ""
        for m in chat:
            from_name = db.execute(sa.text('SELECT name FROM users WHERE nickname=:u'), {'u': m['from_nickname']}).scalar()
            msgs += f"<div class=chat-msg><b>{from_name}:</b> {m['text']} <small>{m['time']}</small></div>"

    content = f"<div class=card><div class=chat-box>{msgs or '<p style=text-align:center;color:gray>No messages yet. Start chatting!</p>'}</div><form method=POST><input id=msg name=msg placeholder='Type message...' required style=width:100%;display:block><button class=btn.gray style=margin-top:8px>Send</button></form></div>"
...
def group_chat(nickname, user, gid):
    with DBSession() as db:
        ...
        # FIX: Pre-fetch all names before closing DB
        msgs = ""
        for m in messages:
            msgs += f"<div class=chat-msg><b>{m['user']}:</b> {m['text']} <small>{m['time']}</small></div>"
...
def admin(nickname, user):
    ...
    if request.method=="POST":
        with DBSession() as db:
            ...
            if "add_question" in request.form:
                ...
                db.execute(sa.text("INSERT INTO questions (key, q, options, ans, exp) VALUES (:k, :q, :o, :a, :e)"),{"k": key, "q": request.form["q"], "o": json.dumps([request.form["a"],request.form["b"],request.form["c"],request.form["d"]]), "a": request.form["ans"], "e": request.form["exp"]}); db.commit(); error = f"<div class=success>Question Added for {sub}</div>"
    ...
    # FIXED: Pass 'this' to selectSubject
    js = Markup(f"""<script>
...
<div class="card"><h2>Add Single Question - CLICK SUBJECT BUTTON</h2><form method="POST">\
...
<label>Select Subject - Click Button Below</label>{subject_grid}<input type=hidden name=admin_subject id=admin_subject required>\
...
<label>Select Class</label><select name="bulk_class" required><option value="">Select Class</option>{"".join([f"<option>{c}</option>" for c in CLASSES])}</select>\
<label>Department</label><input name=bulk_dept placeholder="Science/Commercial/Art or leave blank">\
<label>Subject</label><input name=bulk_subject placeholder="Subject" required>\
<textarea name="bulk_text" rows="10" placeholder="What is 2+2?|3|4|5|6|4|Simple addition" required></textarea>\
