    ...
    if request.method == "POST":
        ...
        with DBSession() as db:
            ...
            if exists: error = "<div class=error>Nickname taken</div>"
                ...
                db.commit()
                success = f"<div class=success>Registration Successful!<br><b>Your Admission No: {admission_no}</b><br>Save it!</div>"
...
@app.route('/group/<int:gid>', methods=["GET","PO    with engine.connect() as conn:
        conn.execute(sa.text("""
        CREATE TABLE IF NOT EXISTS users (
            id SERIAL PRIMARY KEY,
            nickname TEXT UNIQUE,
...
def group_chat(nickname, user, gid):
    with DBSession() as db:
        ...
        emojis = "😀😃😄😁😆😅😂🤣"; emoji_btns = "".join([f"<button type=button class=emoji-btn onclick=\"document.getElementById('msg').value+='{e}'\">{e}</button>" for e in emojis])
        if request.method=="POST":
...
def community(nickname, user):
    is_admin = session.get("admin_logged_in"); emojis = "😀😃😄😁😆😅😂🤣😊😇🙂🙃😉😌😍🥰😘😗😙😚😋😛😝😜🤪🤨🧐🤓😎🤩🥳"
    with DBSession() as db:
        if request.method=="POST":
...
def req_pay(nickname, user, t):
    ...
    form = f"""<div class=card><h2>Pay ₦{price} to unlock</h2><p><b>Bank:</b> {PALMPAY_BANK}<br><b>Account:</b> {PALMPAY_ACCOUNT} <button class=copy-btn onclick=copyAcc()>Copy</button><br><b>Name:</b> {PALMPAY_NAME}</p><form method=POST action=/confirm/{t}><input name=bank_used placeholder="Bank you used to transfer e.g GTBank" required><input name=account_name placeholder="Account Name you used" required><input name=teller_id placeholder="Transaction ID / Teller ID" required><button class=btn>I Have Paid</button></form></div>"""
...
def confirm(nickname, user, t):
    with DBSession() as db:
        db.execute(sa.text("INSERT INTO payments (nickname, name, type, status, bank_used, account_name, teller_id) VALUES (:u, :n, :t, 'Pending', :b, :a, :tid)"),
                   {"u": nickname, "n": user["name"], "t": t, "b": request.form["bank_used"], "a": request.form["account_name"], "tid": request.form["teller_id"]})
        db.commit()
    ...
    if request.method=="POST":
        if "login_pass" in request.form:
            ...
            with DBSession() as db:
                ...
                if "new_notice" in request.form: notices = NOTICES + [request.form["new_notice"]]; ads = ADS + [request.form["new_notice"]]; set_setting("notices", json.dumps(notices)); set_setting("ads", json.dumps(ads)); error = "<div class=success>Notice/Ad Posted to Home</div>"
