...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', "motiz_secret_key_2026_v29")
...
BASE_URL = "https://motiz-e-learning-institution.onrender.com" # YOUR URL
...
ADMIN_PASS = get_setting("admin_pass", ADMIN_PASS) # NOW THIS WILL WORK
...
# ====== BASE HTML WITH FAVICON TOP LEFT ======
...
.header img.logo{{width:35px;height:35px;border-radius:50%;margin-left:5px}}
...
def get_header(nickname,user, show_nav=True):
    ...
    return f"""<div class="header"><img src="{FAVICON_URL}" class="logo">{exit_html}<h1>MOTIZ E-LEARNING {verified}</h1>{theme_html}</div>{nav_html}"""
...
def main(nickname, user):
    ...
    notices_html = "".join([f"<div class='card'><div class=notice-title>📢 {n['title']}</div>{n['text']}<small style='float:right'>{datetime.fromisoformat(n['created_at']).astimezone(NIGERIA_TZ).strftime('%I:%M %p')}</small></div>" for n in NOTICES])
...
def exam(nickname, user):
    ...
    # 1 MONTH LOCK LOGIC: Check if verified and if 30 days passed
...
def start(nickname, user, key, sub):
    ...
    # Total limit depends on verification
    ...
    # ====== NEW TIMER WITH ANTI-CHEAT ======
    timer_js = Markup(f"""let timeLeft = {TIMER_PER_QUESTION};
    const qKey = '{full_key}_{q_index}'; // unique key per question
    ...
    // Load saved time from localStorage so refresh doesn't reset timer
    let savedTime = localStorage.getItem(qKey);
    if(savedTime && parseInt(savedTime) > 0){{ timeLeft = parseInt(savedTime); }}

    function updateTimer(){{
        if(timeLeft < 0) timeLeft = 0; // safety
        localStorage.setItem(qKey, timeLeft); // save time every second
        ...
        if(timeLeft <= 0 &&!submitted){{
            submitted = true;
            localStorage.removeItem(qKey); // clear timer on submit
            let form = document.getElementById('exam_form');
            let input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'save_answer';
            input.value = '1';
    ...
    // Clear timer when user clicks Next/Prev/Submit
    document.getElementById('exam_form').addEventListener('submit', ()=>{{
...
def result(nickname, user):
    ...
    with DBSession() as db:
        # Update counters: free_questions_used for free users, q_used for all
        if user.get('is_verified'):
...
def confirm(nickname, user, t):
    # FIXED: No msg field crash. Only bank_used + account_name
    with DBSession() as db:
...
def lessons(nickname, user):
    # ACCESS: Verified OR lesson_expiry not expired
    access = False
    ...
    if not access:
        content = f'<div class=card><h2>🔒 Unlock Lessons for 15 Days</h2><p><b>Pay &#8358;{LESSON_PRICE}</b></p><p><b>Bank:</b> {PALMPAY_BANK}<br><b>Account Number:</b><input class=readonly-box readonly value="{PALMPAY_ACCOUNT}"><br><b>Account Name:</b> {PALMPAY_NAME}</p><form method=POST action=/confirm/lessons><input name=bank_used placeholder="Bank you used to transfer" required><input name=account_name placeholder="Account Name you used" required><button class=btn><img src="{SEND_BTN_URL}" style="height:20px;vertical-align:middle"> I Have Paid</button></form></div>'
...
def community(nickname, user):
    with DBSession() as db:
        if request.method=="POST":
            if "new_post" in request.form:
                post_text = request.form["post"][:1000] # 1000 CHAR LIMIT FOR POSTS
                ...
                comment_text = request.form["comment_text"][:700] # 700 CHAR LIMIT
    ...
    for p in posts:
        ...
        posts_html += f"<div class=card><b>{p['name']}</b>: {p['text']} {delete_btn}<form method=POST><input type=hidden name=like_post_id value={p['id']}><button class=btn.gray>🩷 {likes_count}</button></form>{comments_html}<form method=POST style='display:flex;gap:5px'><input type=hidden name=comment_post_id value={p['id']}><input name=comment_text placeholder='Write comment...' required maxlength=700 style='flex:1'><button class='btn gray' style='width:45px;height:45px;padding:0'><img src='{SEND_BTN_URL}' style='height:24px;width:24px'></button></form></div>"
    return render_template_string(BASE, title="Community", header=Markup(get_header(nickname,user, show_nav=False)), content=Markup(f"<div class=card><h2>Community</h2><form method=POST><textarea name=post placeholder='Whats on your mind?' required maxlength=1000></textarea><button class=btn><img src='{SEND_BTN_URL}' style='height:20px;vertical-align:middle'> Post</button><input type=hidden name=new_post value=1></form></div>{posts_html}"), timer_script="")
...
def me(nickname, user):
    ...
    referral_card = f"""<div class=card><h2>🔗 Referral Program</h2>
    <p>Share your link. When they pay, you get +5 days FREE Lessons + CBT</p>
    <input class=readonly-box readonly value="{ref_link}" onclick="this.select()">
    <p><b>Friends Referred:</b> {user.get('referral_count',0)}</p>
    <p><b>Free Days Earned:</b> {user.get('free_days',0)}</p>
    </div>"""
...
def dm_chat(nickname, user, to_nickname):
    ...
    with DBSession() as db:
        if request.method=="POST":
            msg_text = request.form["msg"][:700] # 700 CHAR LIMIT
    ...
    content = f"<div class=card><div class=chat-box>{msgs or '<p style=text-align:center;color:gray>No messages yet</p>'}</div></div><form method=POST class=chat-input-fixed><input name=msg placeholder='Type message...' required maxlength=700 style='flex:1'><button class=btn.gray style='width:45px;height:45px;padding:0'><img src='{SEND_BTN_URL}' style='height:28px;width:28px'></button></form>"
...
def group_chat(nickname, user, gid):
    with DBSession() as db:
        ...
        # ADD/REMOVE MEMBER BUTTONS FOR CREATOR ONLY
        member_manage = ""
        if is_creator:
    ...
    content = f"{member_manage}<div class=card><h2>{group['name']}</h2></div><div class=card><div class=chat-box>{msgs or '<p>No messages yet</p>'}</div></div><form method=POST class=chat-input-fixed><input name=msg placeholder='Type message...' required maxlength=700 style='flex:1'><button name=send_msg class=btn.gray style='width:45px;height:45px;padding:0'><img src='{SEND_BTN_URL}' style='height:28px;width:28px'></button></form>"
...
def admin(nickname, user):
    ...
    manage = request.args.get('manage') # for CRUD pages
        ...
        with DBSession() as db:
            ...
            if "add_lesson" in request.form:
                ...
                db.execute(sa.text("INSERT INTO lessons (class, dept, subject, title, notes, date) VALUES (:c, :d, :s, :t, :n, :date)"),
                           {"c": cls, "d": dept, "s": request.form['lesson_subject'], "t": request.form['lesson_title'], "n": request.form['lesson_notes'], "date": str(date.today())}); db.commit(); error = "<div class=success>Lesson Posted</div>"

            # EDIT/DELETE QUESTION - FIXED CORRECT_ANS SELECTED
            ...
            if "edit_lesson" in request.form:
                db.execute(sa.text("UPDATE lessons SET title=:t, notes=:n WHERE id=:id"),{"t": request.form["lesson_title"], "n": request.form["lesson_notes"], "id": request.form["lid"]}); db.commit(); error = "<div class=success>Lesson Updated</div>"
            ...
            if "edit_notice" in request.form:
                ...
                NOTICES[idx] = {"title": request.form["notice_title"], "text": request.form["notice_text"], "created_at": NOTICES[idx]['created_at']}
            ...
            if "post_notice" in request.form:
                NOTICES.insert(0, {"title": request.form["notice_title"], "text": request.form["notice_text"], "created_at": str(datetime.now(NIGERIA_TZ))})
    ...
    # ====== 28 BUTTONS AS PROPER BUTTONS WITH SPACING ======
