...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', "motiz_secret_key_2026_v9_3")
...
def init_db():
    with engine.connect() as conn:
        ...
        """))
        # NEW: FRIEND REQUESTS TABLE - NOT IN PAYMENTS
        conn.execute(sa.text("""
...
.header h1{margin:0;font-size:0.85rem;flex:1;text-align:center;line-height:1.1;white-space:nowrap} /* FIX 2: 1 LINE */
...
.like-btn{background:transparent;border:none;color:#e94560;cursor:pointer;font-weight:bold;margin-right:10px} /* FIX 7: PINK HEART */
...
def lessons(nickname, user):
    ...
    access_banner = f"<div class=success>✅ Access Active till {user['lesson_expiry']}</div>" # FIX 10
...
def group_chat(nickname, user, gid):
    with DBSession() as db:
        ...
        if request.method=="POST":
            if "send_msg" in request.form:
                messages.append({"user": user["name"], "text": request.form["msg"], "time": datetime.now(NIGERIA_TZ).strftime("%H:%M")}) # FIX 1
            ...
            elif "add_member" in request.form and is_creator: # FIX 11
    ...
    with DBSession() as db:
        all_users = db.execute(sa.text("SELECT nickname,name FROM users WHERE nickname!=:u"), {"u": nickname}).mappings().all() # FIX 11
...
@login_required
def dm_chat(nickname, user, to_nickname): # FIX 13: ADDED EMOJI
    ...
    with DBSession() as db:
        if request.method=="POST":
            db.execute(sa.text("INSERT INTO dms (from_nickname, to_nickname, text, time) VALUES (:f, :t, :txt, :time)"),
                       {"f": nickname, "t": to_nickname, "txt": request.form["msg"], "time": datetime.now(NIGERIA_TZ).strftime("%H:%M")}) # FIX 1
    ...
    # BUILD EMOJI TABS FOR DM
    emoji_tabs = "".join([f"<button type=button onclick=\"showEmojiDM('{k}')\" id=dtab_{k}>{k}</button>" for k in EMOJI_GROUPS.keys()])
    emoji_boxes = ""
    for k,v in EMOJI_GROUPS.items():
        btns = "".join([f"<button type=button class=emoji-btn onclick=\"document.getElementById('msg').value+='{e}'\">{e}</button>" for e in v.split()])
        emoji_boxes += f"<div id=dgroup_{k} class=emoji-box>{btns}</div>"

    emoji_js = f"""<script>
    function showEmojiDM(id){{
        document.querySelectorAll('#dm_emoji_wrap.emoji-box').forEach(x=>x.style.display='none');
        document.querySelectorAll('#dm_emoji_tab button').forEach(x=>x.classList.remove('active'));
        document.getElementById('dgroup_'+id).style.display='block';
        document.getElementById('dtab_'+id).classList.add('active');
    }}
    showEmojiDM('{list(EMOJI_GROUPS.keys())[0]}');
    </script>"""
    ...
    content = f"<div class=card><div class=chat-box>{msgs}</div><form method=POST><input id=msg name=msg placeholder='Type message...' required style=width:70%;display:inline-block><button class=btn.gray style=width:28%;display:inline-block>Send</button><button type=button class='btn gray' onclick=document.getElementById('dm_emoji_wrap').style.display='block'>😀</button><div id=dm_emoji_wrap style=display:none><div id=dm_emoji_tab class=emoji-tab>{emoji_tabs}</div>{emoji_boxes}</div></form></div>"
    return render_template_string(BASE, title=f"Chat with {to_name}", header=Markup(get_header(nickname,user)), content=Markup(content), timer_script=Markup(emoji_js))

@app.route('/friends', methods=["GET","POST"]) # FIX 8: NOT USING PAYMENTS TABLE
@login_required
...
@app.route('/start/<path:key>/<path:sub>', methods=["GET","POST"]) # FIX 4 + 5 + 14: NO Q ERROR + PREV NEXT + GRADE AT END
@login_required
...
@app.route('/result')
@login_required
def result(nickname, user): # FIX 5: GRADE ALL AT ONCE
    ...
    with DBSession() as db:
        db.execute(sa.text("UPDATE users SET q_used=q_used+:t, correct=correct+:c WHERE nickname=:u"), {"t": len(questions), "c": correct, "u": nickname})
        db.commit()
...
def community(nickname, user):
    ...
    # BUILD EMOJI TABS FOR COMMUNITY - FIX 6
    ...
    emoji_js = f"""<script>
    function toggleEmojiC(){{document.getElementById('emoji_wrap_c').style.display='block';showEmojiGroupC('{list(EMOJI_GROUPS.keys())[0]}')}} /* FIX 6 */
    function showEmojiGroupC(id){{
        document.querySelectorAll('#emoji_wrap_c.emoji-box').forEach(x=>x.style.display='none'); /* FIX 6: space in selector */
...
@app.route('/admin', methods=["GET","POST"]) # FIX 15 + 16 + 18: EXIT BTN + EDIT/DELETE NOTICE
@login_required
def admin(nickname, user):
    ...
    if request.method=="POST":
        with DBSession() as db:
            ...
            if "new_notice" in request.form: # FIX 18
            ...
            if "delete_notice_id" in request.form: # FIX 18
            ...
            if "edit_notice_id" in request.form: # FIX 18
...
<button name="bulk_upload" class="btn blue">Upload Bulk Questions</button></form></div>'
    return render_template_string(BASE, title="Admin", header=Markup(get_header(nickname,user, show_exit=True)), content=Markup(form), timer_script=Markup(js)) # FIX 15
