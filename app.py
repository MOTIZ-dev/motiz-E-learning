...
def start(nickname, user, key, sub):
    ...
    if request.method == "GET":
        ...
        session[session_key] = questions[:limit]; session[answers_key] = {}; session['q_index'] = 0
    questions = session.get(session_key, []); answers = session.get(answers_key, {}); q_index = session.get('q_index', 0)
    if request.method == "POST":
        if "save_answer" in request.form: session[answers_key][str(q_index)] = request.form.get("answer"); session.modified = True
...
def confirm(nickname, user, t):
    with DBSession() as db: db.execute(sa.text("INSERT INTO payments (nickname, name, type, status, bank_used, account_name) VALUES (:u, :n, :t, 'Pending', :b, :a)"), {"u": nickname, "n": user["name"], "t": t, "b": request.form["bank_used"], "a": request.form["account_name"]}); db.commit()
...
def friends(nickname, user):
    with DBSession() as db:
        if request.method=="POST":
            if "send_request" in request.form: db.execute(sa.text("INSERT INTO friend_requests (from_nickname, to_nickname) VALUES (:f, :t)"), {"f": nickname, "t": request.form["send_request"]}); db.commit()
                ...
                from_user = request.form["accept_user"]; user['friends'].append(from_user)
                db.execute(sa.text("UPDATE users SET friends=:f WHERE nickname=:u"), {"f": json.dumps(user['friends']), "u": nickname})
                from_friends = json.loads(db.execute(sa.text("SELECT friends FROM users WHERE nickname=:u"), {"u": from_user}).scalar() or '[]'); from_friends.append(nickname)
    ...
    incoming_html = "".join([f"<div class=card><b>{r['from_nickname']}</b><form method=POST><input type=hidden name=accept_user value={r['from_nickname']}><button class=btn>Accept</button></form></div>" for r in incoming])
...
def group_chat(nickname, user, gid):
    with DBSession() as db:
        ...
        if request.method=="POST":
            ...
            elif "new_name" in request.form and is_creator: db.execute(sa.text("UPDATE groups SET name=:n WHERE id=:id"), {"n": request.form["new_name"], "id": gid})
        ...
        updated = False
        for m in messages:
            if nickname not in m.get('read_by', []): m['read_by'].append(nickname); updated = True
    ...
    content = f"<div class=card><h2>{group['name']}</h2>{rename_form}{add_member_form}<h4>Members:</h4><ul>{members_html}</ul></div><div class=card><div class=chat-box>{msgs or '<p>No messages yet</p>'}</div><form method=POST><input name=msg placeholder='Type message...' required style=width:70%;display:inline-block><button name=send_msg class=btn.gray style=width:28%;display:inline-block>Send</button></form></div>"
    ...
    if request.method=="POST":
        with DBSession() as db:
            if "verify_id" in request.form:
                ...
                db.commit(); error = "<div class=success>Payment Verified & Saved. User now has access</div>"
            ...
            if "delete_lesson" in request.form: db.execute(sa.text("DELETE FROM lessons WHERE id=:id"), {"id": request.form["delete_lesson"]}); db.commit(); error = "<div class=success>Lesson Deleted</div>"
...
function loadSubjects(){{
    ...
    let key = d? c+'_'+d : c;
    let sDiv = document.getElementById('admin_subject_div');
    let options = (subjects[key] || []).map(function(sub){{ return '<option value="' + sub + '">' + sub + '</option>'; }}).join('');
    sDiv.innerHTML = '<label>Step 3: Select Subject</label><select name=admin_subject required><option value="">Select Subject</option>' + options + '</select>';
}}
...
function loadLessonSubjects(){{
    ...
    let sDiv = document.getElementById('lesson_subject_div');
    let options = (subjects[key] || []).map(function(sub){{ return '<option value="' + sub + '">' + sub + '</option>'; }}).join('');
    sDiv.innerHTML = '<label>Step 3: Select Subject</label><select name=lesson_subject required><option value="">Select Subject</option>' + options + '</select>';
}}
</script>""")
    with DBSession() as db:
    ...
    lessons_admin_html = "".join([f"<div class=card><b>{l['class']} {l['dept']} - {l['subject']}</b>: {l['title']}<form method=POST><input type=hidden name=delete_lesson value={l['id']}><button class='btn red'>Delete Lesson</button></form></div>" for l in all_lessons])
    notices_admin_html = "".join([f"<div class=card><b>{n['title']}</b>: {n['text']}<form method=POST><input type=hidden name=delete_notice value={i}><button class='btn red'>Delete Notice</button></form></div>" for i,n in enumerate(NOTICES)])
...
<div class="card"><h2>Upload Lesson Note - 3 STEP FLOW</h2><form method="POST">\
<label>Step 1: Select Class</label><select name="lesson_class" id="lesson_class" onchange="updateLessonDept()" required><option value="">Select Class</option>{"".join([f"<option>{c}</option>" for c in CLASSES])}</select>\
<div id="lesson_dept_div"></div>\
<div id="lesson_subject_div"></div>\
...
<div class="card"><h2>Manage Lessons</h2>{lessons_admin_html or "<p>No lessons posted</p>"}</div>\
<div class="card"><h2>Manage Notices</h2>{notices_admin_html or "<p>No notices</p>"}</div>\
...
<div class="card"><h2>Add Question - 3 STEP FLOW</h2><form method="POST">\
