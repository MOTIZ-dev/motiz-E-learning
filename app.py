...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', "motiz_secret_key_2026_v28")
...
def start(nickname, user, key, sub):
    ...
    full_key = f"{key}_{sub}" # FIX: Build full key to match admin: SS1_Science_Mathematics
...
def result(nickname, user):
    ...
    for i, q in enumerate(questions): # FIX: Proper loop indentation
...
def lessons(nickname, user):
    ...
    with DBSession() as db:
        # FIX: Query by class only, and also match dept if user has one
        if user.get('dept'):
...
def community(nickname, user):
    ...
    for p in posts:
        ...
        posts_html += f"<div class=card><b>{p['name']}</b>: {p['text']} {delete_btn}<form method=POST><input type=hidden name=like_post_id value={p['id']}><button class=btn.gray>🩷 {likes_count}</button></form>{comments_html}<form method=POST><input type=hidden name=comment_post_id value={p['id']}><input name=comment_text placeholder='Write comment...' required><button class='btn gray'><img src='{SEND_BTN_URL}' style='height:18px;vertical-align:middle'> Send</button></form></div>"
    return render_template_string(BASE, title="Community", header=Markup(get_header(nickname,user, show_nav=False)), content=Markup(f"<div class=card><h2>Community</h2><form method=POST><textarea name=post placeholder='Whats on your mind?' required></textarea><button class=btn><img src='{SEND_BTN_URL}' style='height:20px;vertical-align:middle'> Post</button><input type=hidden name=new_post value=1></form></div>{posts_html}"), timer_script="")
@app.route('/me') # PACKED: PROFILE + CBT STATS + FRIENDS + REFERRAL + LOGOUT
@login_required
def me(nickname, user):
    ...
    # REFERRAL SECTION
    ref_link = f"https://motiz-e-learning-institution.onrender.com/register?ref={nickname}"
...
def dm_chat(nickname, user, to_nickname):
    ...
    content = f"<div class=card><div class=chat-box>{msgs or '<p style=text-align:center;color:gray>No messages yet</p>'}</div><form method=POST style='display:flex;gap:5px'><input name=msg placeholder='Type message...' required style='flex:1'><button class=btn.gray style='width:45px'><img src='{SEND_BTN_URL}' style='height:22px'></button></form></div>"
...
def group_chat(nickname, user, gid):
    with DBSession() as db:
        ...
        if request.method=="POST":
            if "send_msg" in request.form:
                messages.append({"user": user["name"], "text": request.form["msg"], "time": datetime.now(NIGERIA_TZ).strftime("%I:%M %p"), "read_by": [nickname]})
    ...
    content = f"<div class=card><h2>{group['name']}</h2></div><div class=card><div class=chat-box>{msgs or '<p>No messages yet</p>'}</div><form method=POST style='display:flex;gap:5px'><input name=msg placeholder='Type message...' required style='flex:1'><button name=send_msg class=btn.gray style='width:45px'><img src='{SEND_BTN_URL}' style='height:22px'></button></form></div>"
...
def admin(nickname, user):
    ...
    if request.method=="POST":
        with DBSession() as db:
            # 25. CONFIRM PAYMENT + REFERRAL REWARD + VERIFY STUDENT
            if "verify_id" in request.form:
                ...
                if req["type"] == "lessons":
                    db.execute(sa.text("UPDATE users SET lesson_expiry=:d, is_verified=TRUE WHERE nickname=:u"), {"d": str(date.today() + timedelta(days=15)), "u": req["nickname"]}) # 15 DAYS
                # REFERRAL BONUS
                ...
                db.commit(); error = "<div class=success>Payment Verified + Referral Bonus Given + Student Verified</div>"
            ...
            # 1-12. ADD QUESTION - FIXED KEY
            if "add_question" in request.form:
                key = f"{request.form['q_key']}_{request.form['admin_subject']}" # FIX: Save as SS1_Science_Mathematics
            ...
            # 13-24. ADD LESSON - FIXED CLASS/DEPT
            if "add_lesson" in request.form:
                full_key = request.form['l_key'] # e.g. SS1_Science
                parts = full_key.split("_")
                cls = parts[0] # SS1 only
                dept = parts[1] if len(parts) > 1 else "" # Science
    ...
    # ====== 28 BUTTONS IN 4 GROUPS - NO DROPDOWNS ======
...
<div class="card"><h2>📝 GROUP 1: CBT - ADD QUESTIONS</h2><p>12 Buttons</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:5px">{group1}</div></div>\
<div class="card"><h2>🎓 GROUP 2: LESSONS - POST LESSONS</h2><p>12 Buttons</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:5px">{group2}</div></div>\
<div class="card"><h2>👥 GROUP 3: STUDENT MANAGEMENT</h2><h3>25. Confirm Payment</h3>{pending_html or "<p>No pending payments</p>"}<a href=/admin_attendance class=btn.blue>26. View Attendance</a><form method=POST><h3>27. Post Notice</h3><input name=notice_title placeholder="Notice Title" required><textarea name=notice_text placeholder="Notice Message" required></textarea><button name=post_notice class=btn><img src="{SEND_BTN_URL}" style="height:20px;vertical-align:middle"> Post Notice</button></form></div>\
...
def admin_attendance(nickname, user):
    ...
    with DBSession() as db:
        users = db.execute(sa.text("SELECT name, class, dept, q_used, lesson_expiry FROM users ORDER BY class")).mappings().all()
    table = "".join([f"<tr><td>{u['name']}</td><td>{u['class']} {u.get('dept','')}</td><td>{u['q_used']}</td><td>{u['lesson_expiry']}</td></tr>" for u in users])
    content = f"<div class=card><h2>26. View Attendance</h2><table style='width:100%;font-size:0.8rem'><tr><th>Name</th><th>Class</th><th>Q Used</th><th>Lesson Expiry</th></tr>{table}</table></div>"
